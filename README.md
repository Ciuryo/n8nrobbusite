...
- **Desafios práticos validados** — 8 desafios com checklist de critérios **em tempo real** no canvas: construção ("Fluxo de Eco", "Primeiro Agente", "Sessão Persistente Redis", "RAG de Catálogo", "Custom Tool de Frete"), **conserto de fluxos quebrados** ("Agente Sem Cérebro", "RAG Sem Embeddings" — fluxo defeituoso pré-carregado para depurar) e o **desafio BOSS final** combinando agente + Redis + RAG + Custom Tool (+500 XP).
- **Exportação para o n8n real** — botão "⬇ Exportar p/ n8n" baixa o fluxo montado como JSON pronto para importar no n8n de verdade.
